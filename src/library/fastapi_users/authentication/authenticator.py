...
from library.fastapi_users.authentication.strategy import Strategy
...
class Authenticator:
    ...
    def current_user(
        ...
        as FastAPI resolves it statically.
        """
        log.debug(f"{get_enabled_backends=}")
        signature = self._get_dependency_signature(get_enabled_backends)
    ...
    async def _authenticate(
        ...
        **kwargs,
        ...
        for backend in self.backends:
            log.debug(f"backend: {backend}")
            if backend in enabled_backends:
                log.debug("Enabled backend")
        ...
        if user:
            ...
            if active and not user.is_active and not optional:
                log.warning(f"user {user.email} is not active")
                status_code = status.HTTP_401_UNAUTHORIZED
                user = None
                raise HTTPException(status_code=status_code,detail= Message.USER_INACTIVE)
            if superuser and not user.is_superuser and not optional:
                log.warning(f"User {user.email} not a super user")
                raise HTTPException(status_code=status_code, detail=Message.USER_NOT_SUPERUSER)
            elif (
                verified and not user.is_verified and not optional
            ):
                log.warning(f"User {user.email} not verified")
                user = None
                raise HTTPException(status_code=status_code, detail=Message.USER_NOT_VERIFIED)
        if not user and not optional:
            log.error("raising exception")
            raise HTTPException(status_code=status_code, detail = Message.USER_NOT_LOGIN)
        return user, token
