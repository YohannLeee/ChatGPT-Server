...
from library.fastapi_users.db import BaseUserDatabase
from library.fastapi_users.jwt import SecretType, decode_jwt, generate_jwt
from library.fastapi_users.password import PasswordHelper, PasswordHelperProtocol
...
class BaseUserManager(Generic[models.UP, models.ID]):
    ...
    async def get_by_email(self, user_email: str) -> models.UP:
        ...
        log.debug(f"获取用户 {user_email}")

        if user is None:
            log.error(f"User {user_email} not exists")
            raise exceptions.UserNotExists()
        log.debug(f"获取到用户{user.email}")
        return user
    ...
    async def reset_password(
        ...
        try:
            ...
            log.error(f"Invalid ID decoded from token: {user_id}")
            raise exceptions.InvalidResetPasswordToken()
        ...
        if not user.is_active:
            log.error("User {user.email} inactive")
            raise exceptions.UserInactive()
    ...
    async def authenticate(
        ...
        if not verified:
            log.debug(f"用户{user.email}密码验证失败")
            return None
    ...
    async def _update(self, user: models.UP, update_dict: Dict[str, Any]) -> models.UP:
        ...
        for field, value in update_dict.items():
            log.debug(f"Updating users {field} - {value}")
            if field == "email" and value != user.email:
                try:
                    ...
                    log.debug(f"Update email {value} success")
            elif field == "password":
